...
$stmt = $conn->prepare("
    SELECT Post.postID, Post.title, Post.postDate, User.username, COUNT(Upvote.contentNo) AS upvotes 
    FROM Post
    ...
    <body>
        ...
        <div class="container">
            ...
            <div style='margin-bottom: 100px;'>
                ...
                <?php if (!empty($post)) : ?>
                <p> <?= sizeof($post) ?> results was retrieved. </p>
                    <?php foreach ($post as $row): ?>
                        <div class='container border'>
                            <h2><a href='../forum/post.php?postID=<?= $row['postID'] ?>'><?= $row['title'] ?></a></h2>
                            <p>Posted by <b><?= $row['username'] ?></b> on <?= $row['postDate'] ?>
                                <span class='float-right'><?= $row['upvotes'] ?> Upvotes</span></p>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <div class='container'>The post history is clean.</div>
                <?php endif; ?>    
            </div>
        </div>
    </body>
